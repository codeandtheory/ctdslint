<!-- Command: commit -->
<!-- Required Tools: run_terminal_cmd(git status), run_terminal_cmd(git diff), run_terminal_cmd(git log), run_terminal_cmd(git add), run_terminal_cmd(git commit) -->

Generate a well-crafted git commit message following conventional commits and best practices.
...
**DO NOT add co-authoring or attribution lines** (no `Co-authored-by` tags)
